...
router.post("/signup", function (req, res, next) {
  ...
  User.findOne({ email: req.body.email }, function (err, user) {
    if (user) {
      return res
        .status(400)
        .send({ error: true, message: "Bad Request - User already exists" });
    } else {
...
router.post("/me/from/token", function (req, res, next) {
  ...
  if (!token) {
    return res
      .status(401)
      .send({ error: true, message: "You Must Pass a Token!" });
  }
  ...
  jwt.verify(token, process.env.JWT_SECRET, function (err, user) {
    if (err) {
      return res
        .status(500)
        .send({ error: true, message: "JWT Verification Error - " + err });
    }
    ...
    User.findById(
      ...
      function (err, user) {
        if (err) {
          ...
          return res
            .status(500)
            .send({ error: true, message: "Database Error - " + err.message });
        } else if (!user) {
